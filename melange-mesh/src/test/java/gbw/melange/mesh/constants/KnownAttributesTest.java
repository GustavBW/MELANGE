...
class KnownAttributesTest {
    ...
    @Test
    void dynamicDiscoveryTest(){
        IVertAttr[] values = KnownAttributes.values();

        assertTrue(0 < values.length, "The array must be populated.");

        boolean foundNonCandidate = false;
        for(IVertAttr attr : values){
            if(attr == null){
                foundNonCandidate = true;
                break;
            }
        }
        assertFalse(foundNonCandidate, "All entries in array must be valid.");
    }
    @Test
