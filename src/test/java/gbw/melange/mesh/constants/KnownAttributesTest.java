...
class KnownAttributesTest {
    ...
    void convertListToArray() {
        List<VertAttr> attributes = Arrays.asList(KnownAttributes.POSITION, KnownAttributes.COLOR_UNPACKED);
        ...
        for (int i = 0; i < attributes.size(); i++) {
            VertAttr original = attributes.get(i);
    ...
    void convertSetToArray() {
        HashSet<VertAttr> attributeSet = new HashSet<>(Arrays.asList(KnownAttributes.POSITION, KnownAttributes.NORMAL));
    ...
    void convertVertexAttributesToList() {
        ...
        List<VertAttr> convertedList = KnownAttributes.convert(vertexAttributes);
    ...
    void testEquals() {
        ...
        assertTrue(KnownAttributes.POSITION.equals(va), "KnownAttributes should equal the corresponding VertAttr.");
    }

    @Test
    void asVA() {
        VertexAttribute va = KnownAttributes.POSITION.asVA();
        assertNotNull(va, "The method should return a non-null VertAttr.");
